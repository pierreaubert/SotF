...
if [ -f "src-tauri/tauri.conf.json" ]; then
    SIGNING_IDENTITY=$(grep -A 5 '"macOS"' src-tauri/tauri.conf.json | grep 'signingIdentity' | cut -d'"' -f4)
...
if [ "$HAS_DEV_ID" = true ] && [ -n "$APPLE_ID" ] && [ -n "$APPLE_TEAM_ID" ]; then
    ...
    echo "  3. Find the signed DMG in: src-tauri/target/release/bundle/dmg/"
