...
export class CaptureModalManager {
  ...
  private deviceStatusCallback?: (status: 'success' | 'error' | 'neutral') => void;
  ...
  private initializeElements(): void {
    ...
    this.captureModalGraph = document.getElementById('capture_modal_graph') as HTMLCanvasElement;
    this.captureModalPlaceholder = document.getElementById('capture_modal_placeholder');
    this.captureModalProgress = document.getElementById('capture_modal_progress');
    this.captureModalProgressFill = document.getElementById('capture_modal_progress_fill');
    this.captureModalStatus = document.getElementById('capture_modal_status');

    // Buttons
    this.captureModalStart = document.getElementById('capture_modal_start') as HTMLButtonElement;
    this.captureModalStop = document.getElementById('capture_modal_stop') as HTMLButtonElement;
    this.captureModalExport = document.getElementById('capture_modal_export') as HTMLButtonElement;
    this.captureModalCancel = document.getElementById('capture_modal_cancel') as HTMLButtonElement;
    this.captureModalClose = document.getElementById('capture_modal_close') as HTMLButtonElement;

    // Device controls
    this.modalCaptureDevice = document.getElementById('modal_capture_device') as HTMLSelectElement;
    this.modalOutputDevice = document.getElementById('modal_output_device') as HTMLSelectElement;
    this.modalCaptureVolume = document.getElementById('modal_capture_volume') as HTMLInputElement;
    this.modalCaptureVolumeValue = document.getElementById('modal_capture_volume_value');
    this.inputChannelsInfo = document.getElementById('input_channels_info');
    this.modalOutputChannel = document.getElementById('modal_output_channel') as HTMLSelectElement;
    this.modalOutputVolume = document.getElementById('modal_output_volume') as HTMLInputElement;
    this.modalOutputVolumeValue = document.getElementById('modal_output_volume_value');
    this.outputChannelsInfo = document.getElementById('output_channels_info');

    // Signal controls
    this.modalSignalType = document.getElementById('modal_signal_type') as HTMLSelectElement;
    this.modalSweepDuration = document.getElementById('modal_sweep_duration') as HTMLSelectElement;
    this.modalCaptureSampleRate = document.getElementById('modal_capture_sample_rate');
    this.modalCaptureBitDepth = document.getElementById('modal_capture_bit_depth');
    this.modalOutputSampleRate = document.getElementById('modal_output_sample_rate');
    this.modalOutputBitDepth = document.getElementById('modal_output_bit_depth');
    this.modalSweepDurationContainer = document.getElementById('modal_sweep_duration_container');

    // Graph controls
    this.capturePhaseToggle = document.getElementById('capture_phase_toggle') as HTMLInputElement;
    this.captureSmoothingSelect = document.getElementById('capture_smoothing_select') as HTMLSelectElement;
    this.captureCalibrationFile = document.getElementById('capture_calibration_file') as HTMLInputElement;
    this.captureCalibrationBtn = document.getElementById('capture_calibration_btn') as HTMLButtonElement;
    this.captureCalibrationClear = document.getElementById('capture_calibration_clear') as HTMLButtonElement;
    this.captureChannelSelect = document.getElementById('capture_channel_select') as HTMLSelectElement;
    
    // Routing buttons
    this.inputRoutingBtn = document.getElementById('input_routing_btn') as HTMLButtonElement;
    this.outputRoutingBtn = document.getElementById('output_routing_btn') as HTMLButtonElement;
    ...
    this.recordsToggleBtn = document.getElementById('records_toggle') as HTMLButtonElement;
    this.recordsSelectAllBtn = document.getElementById('records_select_all') as HTMLButtonElement;
    this.recordsDeselectAllBtn = document.getElementById('records_deselect_all') as HTMLButtonElement;
    this.recordsDeleteSelectedBtn = document.getElementById('records_delete_selected') as HTMLButtonElement;
  }
  ...
  public setCaptureCompleteCallback(callback: (frequencies: number[], magnitudes: number[]) => void): void {
    this.onCaptureComplete = callback;
  }

  public setOutputDeviceChangeCallback(callback: (deviceId: string) => void): void {
    this.outputDeviceChangeCallback = callback;
  }

  public setDeviceStatusCallback(callback: (status: 'success' | 'error' | 'neutral') => void): void {
  ...
  private handleCaptureError(error: any): void {
    ...
    if (this.captureModalStatus) {
      ...
      if (errorMessage.includes('permission denied')) {
        ...
        statusHTML += '<br><br><div class="error-instructions">📝 <strong>To fix this:</strong><br>1. Connect a microphone to your device<br>2. Check your system audio settings<br>3. Try a different input device</div>';
      } else {
        statusHTML += '<br><br><div class="error-instructions">📝 <strong>Try these steps:</strong><br>1. Refresh the page<br>2. Check your microphone connection<br>3. Try a different browser</div>';
      }
  ...
  private async updateOutputChannelOptions(): Promise<void> {
    ...
    try {
      ...
      if (deviceInfo.outputChannels !== null && deviceInfo.outputChannels !== undefined) {
  ...
  private populateOutputChannels(channelCount: number | null): void {
    if (channelCount === null || channelCount === undefined) {
      console.warn('[CaptureModal] Cannot populate output channels: count is unknown');
      return;
    }
  ...
  private toggleRecordsSidebar(): void {
    ...
    console.log('Records sidebar toggled:', isCollapsed ? 'collapsed' : 'expanded');
  }
    ...
    try {
      ...
      this.currentCaptureData = {
        ...
        metadata: {
          ...
          outputChannel: capture.outputChannel as 'left' | 'right' | 'both' | 'default'
        },
        outputChannel: capture.outputChannel as 'left' | 'right' | 'both' | 'default'
      };
