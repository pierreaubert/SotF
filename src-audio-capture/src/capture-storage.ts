...
export class CaptureStorage {
   ...
   */
  public static saveCapture(captureData: Omit<StoredCapture, 'id' | 'name'>): string {
    ...
    if (captures.length > this.MAX_CAPTURES) {
      ...
      console.log(`Removed ${removed.length} old captures to maintain storage limit`);
    }
  ...
  public static getStats(): CaptureStorageStats {
    ...
    const sorted = [...captures].sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
   ...
   */
  public static toOptimizationFormat(id: string): { frequencies: number[], magnitudes: number[] } | null {
  ...
  private static generateDisplayName(capture: Omit<StoredCapture, 'id' | 'name'>): string {
    const date = capture.timestamp.toLocaleDateString();
    const time = capture.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    const channel = capture.outputChannel === 'both' ? 'Stereo' : 
                   capture.outputChannel === 'left' ? 'Left' :
                   capture.outputChannel === 'right' ? 'Right' : 'Default';
    const signal = capture.signalType.charAt(0).toUpperCase() + capture.signalType.slice(1);
    ...
    try {
      ...
      if (error instanceof Error && error.name === 'QuotaExceededError') {
        ...
        if (captures.length > 3) {
          const reducedCaptures = captures.slice(0, Math.floor(captures.length / 2));
          const reducedData = { ...data, captures: reducedCaptures };
          try {
            localStorage.setItem(this.STORAGE_KEY, JSON.stringify(reducedData));
            console.log(`Reduced storage to ${reducedCaptures.length} captures`);
          } catch (secondError) {
  ...
  private static generateCSV(data: any): string {
    ...
    // Column headers
    lines.push('Frequency(Hz),Raw_SPL(dB),Smoothed_SPL(dB),Raw_Phase(deg),Smoothed_Phase(deg)');
    ...
    for (let i = 0; i < length; i++) {
      ...
      const rawPhase = (data.rawPhase && data.rawPhase[i] !== undefined) ? data.rawPhase[i].toFixed(1) : '0.0';
      const smoothedPhase = (data.smoothedPhase && data.smoothedPhase[i] !== undefined) ? data.smoothedPhase[i].toFixed(1) : '0.0';
      lines.push(`${freq},${rawMag},${smoothedMag},${rawPhase},${smoothedPhase}`);
    }
   ...
   */
  public static getLatestCaptureForChannel(channel: string): StoredCapture | null {
    const captures = this.getAllCaptures();
  ...
  public static performMaintenance(): void {
    ...
    const validCaptures = captures.filter(capture => {
      const isValid = capture.frequencies?.length > 0 &&
    ...
    if (validCaptures.length > this.MAX_CAPTURES) {
      ...
      console.log(`Removed ${excess.length} excess captures during maintenance`);
      changed = true;
    }
