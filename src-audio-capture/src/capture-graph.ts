...
export class CaptureGraphRenderer {
  ...
  private calibrationData: { frequencies: number[], magnitudes: number[] } | null = null;
  ...
  public renderGraph(data: GraphData): void {
    ...
    if (data.frequencies.length > 0) {
      ...
      const calibratedRawMagnitudes = data.rawMagnitudes.length > 0 ? 
        this.applyCalibration(data.frequencies, data.rawMagnitudes) : [];
      
      const calibratedSmoothedMagnitudes = data.smoothedMagnitudes && data.smoothedMagnitudes.length > 0 ? 
        this.applyCalibration(data.frequencies, data.smoothedMagnitudes) : [];
      
      const normalizedRawMagnitudes = calibratedRawMagnitudes.length > 0 ? 
        CaptureGraphRenderer.normalizeMagnitudes(data.frequencies, calibratedRawMagnitudes) : [];
      
      const normalizedSmoothedMagnitudes = calibratedSmoothedMagnitudes.length > 0 ? 
        CaptureGraphRenderer.normalizeMagnitudes(data.frequencies, calibratedSmoothedMagnitudes) : null;
      ...
      if (this.showCombined) {
        if (normalizedRawMagnitudes.length > 0) {
          ...
          this.drawMagnitudeCurve(data.frequencies, normalizedRawMagnitudes, this.rawMagnitudeColor, 1, 'raw', 'Combined');
        }
        
        if (normalizedSmoothedMagnitudes && normalizedSmoothedMagnitudes.length > 0) {
          console.log('Drawing smoothed magnitude curve (normalized)');
          this.drawMagnitudeCurve(data.frequencies, normalizedSmoothedMagnitudes, this.smoothedMagnitudeColor, 3, 'smoothed', 'Combined');
        }
      }
      ...
      if (data.channelData) {
        ...
        if (this.showLeft && data.channelData.left) {
          const leftCalibratedRaw = this.applyCalibration(data.frequencies, data.channelData.left.rawMagnitudes);
          const leftNormalizedRaw = CaptureGraphRenderer.normalizeMagnitudes(data.frequencies, leftCalibratedRaw);
          
          this.drawMagnitudeCurve(data.frequencies, leftNormalizedRaw, this.leftChannelColor, 1, 'raw', 'Left');
          
          if (data.channelData.left.smoothedMagnitudes) {
            const leftCalibratedSmoothed = this.applyCalibration(data.frequencies, data.channelData.left.smoothedMagnitudes);
            const leftNormalizedSmoothed = CaptureGraphRenderer.normalizeMagnitudes(data.frequencies, leftCalibratedSmoothed);
            this.drawMagnitudeCurve(data.frequencies, leftNormalizedSmoothed, this.leftChannelColor, 2, 'smoothed', 'Left');
          }
        }
        
        // Right channel  
        if (this.showRight && data.channelData.right) {
          const rightCalibratedRaw = this.applyCalibration(data.frequencies, data.channelData.right.rawMagnitudes);
          const rightNormalizedRaw = CaptureGraphRenderer.normalizeMagnitudes(data.frequencies, rightCalibratedRaw);
          
          this.drawMagnitudeCurve(data.frequencies, rightNormalizedRaw, this.rightChannelColor, 1, 'raw', 'Right');
          
          if (data.channelData.right.smoothedMagnitudes) {
            const rightCalibratedSmoothed = this.applyCalibration(data.frequencies, data.channelData.right.smoothedMagnitudes);
            const rightNormalizedSmoothed = CaptureGraphRenderer.normalizeMagnitudes(data.frequencies, rightCalibratedSmoothed);
            this.drawMagnitudeCurve(data.frequencies, rightNormalizedSmoothed, this.rightChannelColor, 2, 'smoothed', 'Right');
          }
        }
        
        // Average channel
        if (this.showAverage && data.channelData.average) {
          const avgCalibratedRaw = this.applyCalibration(data.frequencies, data.channelData.average.rawMagnitudes);
          const avgNormalizedRaw = CaptureGraphRenderer.normalizeMagnitudes(data.frequencies, avgCalibratedRaw);
          
          this.drawMagnitudeCurve(data.frequencies, avgNormalizedRaw, this.averageChannelColor, 1, 'raw', 'Average');
          
          if (data.channelData.average.smoothedMagnitudes) {
            const avgCalibratedSmoothed = this.applyCalibration(data.frequencies, data.channelData.average.smoothedMagnitudes);
            const avgNormalizedSmoothed = CaptureGraphRenderer.normalizeMagnitudes(data.frequencies, avgCalibratedSmoothed);
            this.drawMagnitudeCurve(data.frequencies, avgNormalizedSmoothed, this.averageChannelColor, 2, 'smoothed', 'Average');
          }
        }
      }
      ...
      if (this.showPhase) {
        if (data.rawPhase && data.rawPhase.length > 0) {
          this.drawPhaseCurve(data.frequencies, data.rawPhase, this.rawPhaseColor, 1, 'raw');
        }
        
        if (data.smoothedPhase && data.smoothedPhase.length > 0) {
          this.drawPhaseCurve(data.frequencies, data.smoothedPhase, this.smoothedPhaseColor, 3, 'smoothed');
        }
      }
  ...
  public setCalibrationData(frequencies: number[], magnitudes: number[]): void {
    console.log('Setting calibration data:', { frequencies: frequencies.length, magnitudes: magnitudes.length });
  ...
  public setChannelVisibility(channel: 'left' | 'right' | 'average' | 'combined', visible: boolean): void {
    switch (channel) {
      case 'left':
  ...
  public getChannelVisibility(channel: 'left' | 'right' | 'average' | 'combined'): boolean {
    switch (channel) {
      case 'left': return this.showLeft;
      case 'right': return this.showRight;
      case 'average': return this.showAverage;
      case 'combined': return this.showCombined;
  ...
  public renderPlaceholder(): void {
    ...
    this.ctx.fillText('Click "Start Capture" to begin measurement', centerX, centerY);
  }
  ...
  public updateProgress(progress: number, currentFreq?: number): void {
    ...
    if (currentFreq) {
      ...
      this.ctx.textAlign = 'center';
      this.ctx.fillText(`${Math.round(currentFreq)} Hz`, progressX, rect.top - 10);
    }
  }

  private clear(): void {
    this.ctx.clearRect(0, 0, this.width / window.devicePixelRatio, this.height / window.devicePixelRatio);
  }
  ...
  private getPlotArea(): { left: number; top: number; width: number; height: number; right: number; bottom: number } {
  ...
  private drawLabels(): void {
    ...
    // X-axis title
    this.ctx.fillText('Frequency (Hz)', rect.left + rect.width / 2, rect.bottom + 50);
    ...
    const title = this.showPhase ? 'Frequency & Phase Response' : 'Frequency Response';
    this.ctx.fillText(title, rect.left + rect.width / 2, 30);
  }

  private drawMagnitudeCurve(frequencies: number[], magnitudes: number[], color: string, lineWidth: number, type: 'raw' | 'smoothed', channelLabel?: string): void {
    const rect = this.getPlotArea();
    ...
    for (let i = 0; i < frequencies.length; i++) {
      ...
      if (!freqValid || !magValid) {
        outOfRangePoints++;
        if (pointsDrawn < 5) {
          console.log(`  Skipped point ${i}: freq=${freq} (valid=${freqValid}), mag=${mag} (valid=${magValid})`);
        }
        continue;
      }
      ...
      if (pointsDrawn < 5) {
        console.log(`  Point ${pointsDrawn}: freq=${freq}, mag=${mag}, x=${x} (inBounds=${xInBounds}), y=${y} (inBounds=${yInBounds})`);
        console.log(`    Rect bounds: left=${rect.left}, right=${rect.left + rect.width}, top=${rect.top}, bottom=${rect.top + rect.height}`);
      }
    ...
    console.log(`  Valid points: ${validPoints}, Out of range: ${outOfRangePoints}, Total processed: ${frequencies.length}`);
    ...
    if (type === 'raw' && validPoints > 0) {
      ...
      for (let i = 0; i < frequencies.length && circlesDrawn < 5; i++) {
        ...
        if (freq >= 20 && freq <= 20000 && !isNaN(mag) && isFinite(mag)) {
          ...
          console.log(`Drew debug circle at (${x}, ${y}) for freq=${freq}, mag=${mag}`);
        }
      }
  ...
  private drawPhaseCurve(frequencies: number[], phases: number[], color: string, lineWidth: number, type: 'raw' | 'smoothed'): void {
    const rect = this.getPlotArea();
    this.ctx.strokeStyle = color;
    this.ctx.lineWidth = lineWidth;
    this.ctx.lineCap = 'round';
  ...
  private drawLegend(data: GraphData): void {
    ...
    this.ctx.fillRect(legendX - 10, legendY - 10, legendWidth + 20, legendHeight);
    this.ctx.strokeStyle = this.gridColor;
    this.ctx.lineWidth = 1;
    this.ctx.strokeRect(legendX - 10, legendY - 10, legendWidth + 20, legendHeight);
    ...
    const getChannelSuffix = (outputChannel?: string) => {
      if (!outputChannel || outputChannel === 'both' || outputChannel === 'default') {
        return '';
      }
    ...
    if (this.showCombined) {
      ...
      if (data.smoothedMagnitudes) {
        ...
        this.ctx.fillStyle = this.textColor;
        this.ctx.fillText(`Smoothed${channelSuffix}`, currentX + 25, currentY + 4);
        currentX += itemSpacing;
      }
    }
  ...
  private dbToY(db: number, rect: { top: number; height: number; bottom: number }): number {
    const minDb = -40;
    const maxDb = 10;
  ...
  private phaseToY(phase: number, rect: { top: number; height: number; bottom: number }): number {
    const minPhase = -180;
    const maxPhase = 180;
  ...
  private applyCalibration(frequencies: number[], magnitudes: number[]): number[] {
  ...
  public static normalizeMagnitudes(frequencies: number[], magnitudes: number[]): number[] {
    ...
    if (count === 0) {
      console.warn('No frequencies found in 100Hz-10kHz range for normalization');
      return magnitudes.slice(); // Return copy of original
    }
    
    const mean = sum / count;
    console.log(`Normalization: mean over 100Hz-10kHz = ${mean.toFixed(2)} dB (${count} points)`);
    
    // Subtract the mean from all magnitudes
    const normalized = magnitudes.map(mag => mag - mean);
    
    console.log(`Magnitude range before normalization: ${Math.min(...magnitudes).toFixed(1)} to ${Math.max(...magnitudes).toFixed(1)} dB`);
    console.log(`Magnitude range after normalization: ${Math.min(...normalized).toFixed(1)} to ${Math.max(...normalized).toFixed(1)} dB`);
    
    return normalized;
  }

  // Smoothing algorithm - 1/3 octave smoothing for magnitude
  public static applySmoothing(frequencies: number[], magnitudes: number[], octaveFraction: number = 3): number[] {
  ...
  public static applyPhaseSmoothing(frequencies: number[], phases: number[], octaveFraction: number = 3): number[] {
  ...
  public testRender(): void {
    ...
    console.log('Test data generated:', { frequencies: frequencies.length, magnitudes: magnitudes.length });
    console.log('First 5 frequencies:', frequencies.slice(0, 5));
    console.log('First 5 magnitudes:', magnitudes.slice(0, 5));
