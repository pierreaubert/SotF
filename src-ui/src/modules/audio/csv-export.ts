...
export class CSVExporter {
  ...
  private static generateCSV(data: ExportData): string {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (hasPhase) {
        const rawPhase = (data.rawPhase![i] !== undefined) ? data.rawPhase![i].toFixed(1) : '0.0';
        const smoothedPhase = (data.smoothedPhase![i] !== undefined) ? data.smoothedPhase![i].toFixed(1) : '0.0';
  ...
  private static generateFilename(metadata: CaptureMetadata): string {
    ...
    const time = metadata.timestamp.toISOString().split('T')[1].split('.')[0].replace(/:/g, '-'); // HH-MM-SS
  ...
  public static validateExportData(data: Partial<ExportData>): string[] {
    ...
    if (data.frequencies && data.rawMagnitudes && data.smoothedMagnitudes) {
      ...
      if (freqLen !== rawLen || freqLen !== smoothLen) {
        errors.push(`Data length mismatch: frequencies(${freqLen}), raw(${rawLen}), smoothed(${smoothLen})`);
      }
    }
   ...
   */
  public static generatePreview(data: ExportData, maxLines: number = 10): string {
    const csv = this.generateCSV(data);
    ...
    if (lines.length > previewLines.length) {
      previewLines.push(`... and ${lines.length - previewLines.length} more rows`);
    }
   ...
   */
  public static toOptimizationFormat(data: ExportData): { frequencies: number[], magnitudes: number[] } {
    return {
