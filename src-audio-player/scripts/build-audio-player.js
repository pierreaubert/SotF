...
try {
  ...
  fs.copyFileSync(
    path.join(AUDIO_PLAYER_DIR, "README.md"),
  ...
  try {
    execSync(
      `npx tsc ${path.join(BUILD_DIR, "audio-player.ts")} --target es2020 --module es2020 --lib es2020,dom --skipLibCheck --moduleResolution node --esModuleInterop --outDir ${BUILD_DIR}`,
      {
        stdio: "inherit",
      },
    );
