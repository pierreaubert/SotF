            ...
            if let Ok(name) = device.name() {
                ...
                if let Ok(configs) = device.supported_input_configs() {
                    for config in configs {
                        ...
                        for sample_rate in [config_range.min_sample_rate().0, config_range.max_sample_rate().0] {
                            // Common channel configurations
                            for &channels in &[config_range.channels(), 1, 2] {
                ...
                let default_config = device.default_input_config().ok().map(|config| {
                    AudioConfig {
...
pub async fn set_audio_device(
    ...
    println!("[AUDIO DEBUG] Setting {} device '{}' with config: sample_rate={}, channels={}, format={}",
            ...
            Ok(configs) => {
                ...
                for supported_config in configs {
                    ...
                    if supported_config.min_sample_rate() <= sample_rate
                        ...
                        && format_to_string(supported_config.sample_format()) == config.sample_format {
                        valid = true;
                        break;
                    }
                }
    ...
    if !config_valid {
        eprintln!("[AUDIO ERROR] Invalid configuration for device '{}': sample_rate={}, channels={}, format={}",
...
#[tauri::command]
pub async fn get_audio_config(audio_state: State<'_, SharedAudioState>) -> Result<AudioState, String> {
...
#[tauri::command]
pub async fn get_device_properties(device_name: String, is_input: bool) -> Result<serde_json::Value, String> {
    println!("[AUDIO DEBUG] Getting properties for {} device: {}",
             if is_input { "input" } else { "output" },
             device_name);
